...
uint32_t StX=0; // position along the horizonal axis 0 to 20plot
...
void setPlotParams(uint32_t xm, uint32_t xmi, uint32_t ym, uint32_t ymi){
	currentPlotParams.xmax = xm;
	currentPlotParams.xmin = xmi;
	currentPlotParams.ymax = ym;
	currentPlotParams.ymin = ymi;
}

uint16_t plot_data[4096];

void ST7735_plotInit(uint32_t minX, uint32_t maxX, uint32_t minY, uint32_t maxY, char* title) {
	ST7735_InitR(INITR_REDTAB);
	ST7735_FillRect(0, 0, 128, 20, ST7735_BLACK);
	ST7735_DrawString(0, 0, title, ST7735_YELLOW);
	ST7735_FillRect(0, 20, 128, 140, ST7735_WHITE);
	setPlotParams(maxX, minX, maxY, minY);
	for(int i = 0; i < 4096; i++) {
		plot_data[i] = 0;
	}
}

void ST7735_plotPmf(uint32_t* data, int count, int thickness){
	uint16_t max = 0;
	uint16_t first = 0xFFFF, last = 4096;
	//get data for plot and also find max value
	for(int i = 0; i < count; i++) {
		plot_data[data[i]]++;
		if(plot_data[data[i]] > max) {
			max = plot_data[data[i]];
		}
	}
	//find the range of data so we can autozoom the plot
	for(int i = 0; i < 4096; i++) {
		if(plot_data[i] != 0) {
			last = i;
			if(first == 0xFFFF) {
	...
	if((last - first) < 127/thickness) {
		last += (127/thickness)/2;
		first -= (127/thickness)/2;
	}
	for(int i = first; i <= last; i++){
		for(int j = 0; j < thickness; j++){
			ST7735_DrawFastHLine(0, 32+(127*(last - (i))/(last - first)) + j, (127*(plot_data[i])/(max*5/4)), ST7735_BLUE);
		}
	}	
}
